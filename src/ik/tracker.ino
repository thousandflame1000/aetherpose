...
// --- BLE 設定 ---
// [修改] 縮短名稱以確保能放入廣播封包中，解決名稱在 "Arduino" 和 "Aetherpose..." 間跳動的問題
#define BLE_DEVICE_NAME "Aether"
...
struct __attribute__((packed)) FullDataPacket {
  ...
  uint8_t id = 1;        // Tracker ID (改回 1，配合 100% 電量測試)
...
public:
    ...
    float beta = 0.3f; // [修改] 提高增益 (0.1 -> 0.3) 以增加動態響應，減少快速移動時的延遲感
...
void setup() {
  ...
  // [新增] 設定藍牙發射功率為最大 (+4dBm) 以提升連線穩定性
  // BLE.setTxPower(4); // 暫時註解掉：您的 ArduinoBLE 函式庫版本不支援此功能，這不影響主要的斷線修復
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      if (millis() - last_send_time >= SEND_INTERVAL_MS) {
        ...
        // [修改] 只有當中央裝置訂閱了通知才發送數據，避免緩衝區溢位導致斷線
        if (dataCharacteristic.subscribed()) {
          packet.sequence++;
          // 組裝 framed 封包: [0xAA,0x55][len:u8][payload...][crc16_le:u16]
          ...
          // 計算 CRC16-CCITT (false) over payload
      ...
      // [新增] 短暫延遲以讓出 CPU 時間給 BLE 堆疊處理背景任務，防止連線逾時
      delay(5); // [修改] 稍微增加延遲至 5ms，讓連線更穩定
    }
    ...
    // [重要修正] 斷線後必須手動重新啟動廣播，否則電腦無法再次搜尋到或連線
    // 先停止廣播確保狀態重置
    // BLE.stopAdvertise(); // [修改] 移除這行，直接 advertise 即可，避免狀態錯亂
    BLE.advertise();
  }
  // 如果沒有連線，LED 會呈現呼吸燈效果
  else {
    ...
    // [新增] 關鍵修正：在未連線時給予延遲，讓 BLE 堆疊有時間處理連線請求
    // 如果沒有這行，CPU 會被佔滿，導致無法建立連線
    delay(50); // 稍微增加延遲至 50ms，確保連線握手更穩定
  }
}
...
uint8_t getBatteryPercentage() {
  return 100; // [除錯] 強制回傳 100% 以排除因電量為 0 導致 UI 判斷裝置離線的問題
  /*
  // 1. 讀取類比腳位的原始值
