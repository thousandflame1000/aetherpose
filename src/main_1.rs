...
struct AppConfig {
    ...
    #[serde(default)]
    theme_custom: Option<crate::theme::ThemeDef>,
...
fn default_theme_custom() -> Option<crate::theme::ThemeDef> { None }
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            ...
            smoothing_beta: 3.0, // 提高預設速度係數，讓快速動作更靈敏
            leg_ratio: default_leg_ratio(), // Default leg ratio
            floor_offset: 0.0,
...
fn main() -> eframe::Result<()> {
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            ...
            let mut skeleton = match backend_shared_skel.read() {
                Ok(s) => s.clone(),
                Err(e) => {
                    error!("backend_shared_skel read poisoned: {}", e);
                    skeleton::model::SkeletonModel::new_humanoid()
                }
            };
    ...
    eframe::run_native(
        ...
        Box::new(|cc| {
            ...
            // apply initial style based on config: custom theme (if present) or variant
            let theme = if let Some(def) = &config.theme_custom { crate::theme::Theme::from_def(def) } else { crate::theme::Theme::from_variant(config.theme_variant) };
...
struct AetherposeApp {
    ...
    // 可調整側邊欄寬度
    sidebar_width: f32,
    // hover animation state per sidebar tab (scale, target)
    sidebar_hover_scale: [f32; 4],
    sidebar_hover_target: [f32; 4],
...
impl AetherposeApp {
    fn new(
        ...
        Self {
            ...
            sidebar_hover_scale: [1.0, 1.0, 1.0, 1.0],
            sidebar_hover_target: [1.0, 1.0, 1.0, 1.0],
...
impl eframe::App for AetherposeApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // update hover animation state for sidebar (per-tab easing)
        let mut any_anim = false;
        for i in 0..4 {
            let diff = self.sidebar_hover_target[i] - self.sidebar_hover_scale[i];
            if diff.abs() > 0.001 {
                self.sidebar_hover_scale[i] += diff * 0.35;
                any_anim = true;
            } else {
                self.sidebar_hover_scale[i] = self.sidebar_hover_target[i];
            }
        }
        if any_anim {
            ctx.request_repaint();
        ...
        let theme = if let Some(def) = &self.config.theme_custom { crate::theme::Theme::from_def(def) } else { crate::theme::Theme::from_variant(self.theme_variant) };
        ...
        egui::SidePanel::left("sidebar_panel")
            ...
            .show(ctx, |ui| {
                ...
                let mut tab_btn = |label: &str, tab: Tab| {
                    ...
                    let idx = match tab { Tab::Calibration => 0, Tab::Monitor => 1, Tab::Body => 2, Tab::System => 3 };
                    let scale = self.sidebar_hover_scale[idx];
                    let btn = egui::Button::new(egui::RichText::new(icon).size(theme.icon_size * scale).color(if selected { theme.foreground } else { theme.hint_text }))
                        .fill(fill)
                        .stroke(stroke)
                        .rounding(egui::Rounding::same(theme.corner_round_md));
                    let resp = ui.add_sized([ui.available_width(), btn_h], btn).on_hover_text(label);
                    // hover visual: stronger drop shadow + subtle outer glow
                    if resp.hovered() {
                        // set animation target to lifted scale for this tab
                        self.sidebar_hover_target[idx] = 1.12;
                        // multi-layer shadow to simulate blur + lift
                        let base_alpha = 60u8;
                        for (i, (offset_y, alpha_mul, expand)) in [(theme.shadow_offset_y, 1.0, 2.0), (theme.shadow_offset_y * 0.6, 0.6, 4.0), (theme.shadow_offset_y * 0.3, 0.3, 8.0)].iter().enumerate() {
                            let a = (base_alpha as f32 * alpha_mul) as u8;
                            let shadow_color = egui::Color32::from_rgba_unmultiplied(0, 0, 0, a);
                            let shadow_rect = resp.rect.translate(egui::vec2(0.0, *offset_y));
                            ui.painter().rect_filled(shadow_rect.expand(*expand), egui::Rounding::same(theme.corner_round_md + (i as f32)), shadow_color);
                        }
                        // subtle outer glow using selection color
                        let sel = theme.selection;
                        let glow = egui::Color32::from_rgba_unmultiplied(sel.r(), sel.g(), sel.b(), 36);
                        let glow_rect = resp.rect.expand(10.0);
                        ui.painter().rect_filled(glow_rect, egui::Rounding::same(theme.corner_round_md + 3.0), glow);
                        // slight inner stroke to simulate lift
                        let inner = resp.rect.shrink(2.0);
                        ui.painter().rect_stroke(inner, egui::Rounding::same((theme.corner_round_md - 1.0).max(0.0)), egui::Stroke::new(theme.stroke_w, theme.btn_primary));
                    } else {
                        // lower the target for this tab when not hovered
                        self.sidebar_hover_target[idx] = 1.0;
                    }
                    if resp.clicked() {
                ...
                // Theme 切換區（放在 tab 按鈕下方，普通 top-down layout）
                egui::ScrollArea::vertical().id_salt("sidebar_theme_scroll").show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let mut changed = false;
                            ...
                            for (v, icon) in variants.iter() {
                                ...
                                let resp = ui.add_sized([theme.icon_btn_w, theme.btn_h], egui::Button::new(*icon).fill(fill).rounding(egui::Rounding::same(theme.corner_round_md))).on_hover_text(label);
                                if resp.clicked() {
                                    self.theme_variant = *v;
                                    // selecting a built-in variant clears any custom theme
                            ...
                            ui.vertical(|ui| {
                                ui.horizontal(|ui| {
                                    let _ = ui.add_sized([theme.swatch_w, theme.swatch_h], egui::Button::new(" ").fill(theme.window_fill).stroke(egui::Stroke::new(theme.stroke_w, theme.border)));
                                    let _ = ui.add_sized([theme.swatch_w, theme.swatch_h], egui::Button::new(" ").fill(theme.panel_fill).stroke(egui::Stroke::new(theme.stroke_w, theme.border)));
                                    let _ = ui.add_sized([theme.swatch_w, theme.swatch_h], egui::Button::new(" ").fill(theme.canvas_fill).stroke(egui::Stroke::new(theme.stroke_w, theme.border)));
                                });
                                ui.label(egui::RichText::new(self.i18n.t("theme.preview_hint")).size(theme.font_small).weak());

                                // Theme customization UI: edit numeric metrics and apply/save
                                ui.add_space(theme.gap_xs);
                                ui.collapsing(self.i18n.t("theme.customize"), |ui| {
                                    // start from config.custom if present else from current theme
                                    let mut working_def = if let Some(d) = &self.config.theme_custom { d.clone() } else { theme.to_def() };

                                    ui.horizontal(|ui| {
                                        ui.label(self.i18n.t("theme.font_reg"));
                                        ui.add(egui::DragValue::new(&mut working_def.font_reg).speed(0.5));
                                        ui.label(self.i18n.t("theme.font_small"));
                                        ui.add(egui::DragValue::new(&mut working_def.font_small).speed(0.5));
                                    });

                                    ui.horizontal(|ui| {
                                        ui.label(self.i18n.t("theme.corner_round_md"));
                                        ui.add(egui::DragValue::new(&mut working_def.corner_round_md).speed(0.5));
                                        ui.label(self.i18n.t("theme.corner_round_sm"));
                                        ui.add(egui::DragValue::new(&mut working_def.corner_round_sm).speed(0.5));
                                    });

                                    ui.horizontal(|ui| {
                                        ui.label(self.i18n.t("theme.icon_btn_w"));
                                        ui.add(egui::DragValue::new(&mut working_def.icon_btn_w).speed(1.0));
                                        ui.label(self.i18n.t("theme.shadow_offset_y"));
                                        ui.add(egui::DragValue::new(&mut working_def.shadow_offset_y).speed(0.5));
                                    });

                                    ui.horizontal(|ui| {
                                        ui.label(self.i18n.t("theme.table_min_col_width"));
                                        ui.add(egui::DragValue::new(&mut working_def.table_min_col_width).speed(1.0));
                                        ui.label(self.i18n.t("theme.scroll_max_h"));
                                        ui.add(egui::DragValue::new(&mut working_def.scroll_max_h).speed(1.0));
                                    });

                                    ui.horizontal(|ui| {
                                        if ui.add(egui::Button::new(self.i18n.t("apply"))).clicked() {
                                            let t = crate::theme::Theme::from_def(&working_def);
                                            configure_style(ctx, &t);
                                        }
                                        if ui.add(egui::Button::new(self.i18n.t("theme.save_custom"))).clicked() {
                                            self.config.theme_custom = Some(working_def.clone());
                                            // also persist variant marker
                                            self.config.theme_variant = crate::theme::ThemeVariant::Solarized;
                                            self.config.save();
                                        }
                                        if ui.add(egui::Button::new(self.i18n.t("theme.clear_custom"))).clicked() {
                                            self.config.theme_custom = None;
                                            self.config.save();
                            ...
                            if ui.add(egui::Button::new(ui_icons::ICON_EXPORT).small()).on_hover_text(self.i18n.t("theme.export")).clicked() {
                                let theme_inst = if let Some(def) = &self.config.theme_custom { crate::theme::Theme::from_def(def) } else { crate::theme::Theme::from_variant(self.theme_variant) };
                            ...
                            if ui.add(egui::Button::new(ui_icons::ICON_IMPORT).small()).on_hover_text(self.i18n.t("theme.import")).clicked() {
                                match crate::theme::Theme::import_from_file("theme_custom.json") {
                                    Ok(t) => {
                                        // apply imported theme and persist as custom ThemeDef
                                        configure_style(ctx, &t);
                    ...
                    }); // end sidebar_theme_scroll ScrollArea

                // 狀態列（普通 layout，不用 bottom_up）
                ui.separator();
                ui.horizontal(|ui| {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            match self.current_tab {
                ...
                Tab::Monitor => {
                    ...
                    ui.label(egui::RichText::new(self.i18n.t("monitor.scroll_hint")).size(theme.font_small).weak());
                            ...
                            .show(ui, |ui| {
                                ...
                                for t in tracker_list {
                                    ...
                                    egui::ComboBox::from_id_salt(t.id)
                                            .selected_text(match t.assigned_bone {
                                                None => self.i18n.t("tracker.unassigned"),
                                                Some(bid) => bone_name(&self.i18n, bid),
                                            })
                                        .show_ui(ui, |ui| {
                                            // 定義可選的部位列表
                                            // 未分配選項 (用 255 作為 sentinel，避免與 bone_id=0 的臀部衝突)
                                            let unassigned_sentinel: u8 = 255;
                                            let mut combo_val = t.assigned_bone.unwrap_or(unassigned_sentinel);
                                            ...
                                            for &(bid, name_key) in options {
                                                ...
                                                ui.selectable_value(&mut combo_val, bid, label);
                                            }

                                            if combo_val != prev_val {
                                                if combo_val == unassigned_sentinel {
                                                    self.config.tracker_assignments.remove(&t.id);
                                                } else {
                                                    let _ = self.cmd_tx.send(BackendCommand::AssignTracker(t.id, combo_val));
                                                    self.config.tracker_assignments.insert(t.id, combo_val);
                                                }
                                            }
                                        });
                ...
                Tab::System => {
                    ...
                    ui.group(|ui| {
                        ...
                        egui::Grid::new("settings_serial_grid").spacing(egui::vec2(GAP_SM, GAP_SM)).show(ui, |ui| {
                            ...
                            ui.horizontal(|ui| {
                                let _ = ui.add(egui::TextEdit::singleline(&mut self.serial_port_edit).desired_width(theme.input_w_large));
                            });
                            ui.end_row();

                            ui.label(self.i18n.t("serial.baud"));
                            ui.horizontal(|ui| {
                                let _ = ui.add(egui::TextEdit::singleline(&mut self.serial_baud_edit).desired_width(theme.input_w_med));
                            });
                            ui.end_row();
                        });
                        ...
                        egui::ScrollArea::vertical().max_height(theme.scroll_max_h).show(ui, |ui| {
                            let is_err = |s: &str| {
                        ...
                        ui.horizontal(|ui| {
                            if ui_icons::icon_button(ui, ui_icons::ICON_EXPORT, &self.i18n.t("serial.export_serial_log")).clicked() {
                                ...
                                if let Ok(data) = std::fs::read(src) {
                                    ...
                                    if std::fs::write(&fname, data).is_ok() {
                    ...
                    ui.group(|ui| {
                        ...
                        egui::Grid::new("settings_zupt_grid").spacing(egui::vec2(GAP_SM, GAP_SM)).show(ui, |ui| {
                            ...
                            ui.horizontal(|ui| {
                                let _ = ui.add(egui::TextEdit::singleline(&mut self.zupt_window_edit).desired_width(theme.input_w_med));
                            });
                            ui.end_row();

                            ui.label(self.i18n.t("zupt.accel_var"));
                            ui.horizontal(|ui| {
                                let _ = ui.add(egui::TextEdit::singleline(&mut self.zupt_accel_var_edit).desired_width(theme.input_w_med));
                            });
                            ui.end_row();

                            ui.label(self.i18n.t("zupt.gyro_thresh"));
                            ui.horizontal(|ui| {
                                let _ = ui.add(egui::TextEdit::singleline(&mut self.zupt_gyro_edit).desired_width(theme.input_w_med));
                            });
                            ui.end_row();
                        });
                    ...
                    ui.group(|ui| {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            egui::Frame::group(ui.style()).show(ui, |ui| {
                                    ...
                                    egui::ComboBox::from_label("")
                                        ...
                                        .show_ui(ui, |ui| {
                                            for (code, disp) in self.i18n.available_langs_with_display() {
                                                if ui.selectable_label(self.lang == code, &disp).clicked() && self.i18n.set_lang(&code) {
                                ...
                                ui.add_space(GAP_XS);
                                ui.label(egui::RichText::new(self.i18n.t("recorder.tip_empty_name")).size(theme.font_small).weak());

                                ui.horizontal(|ui| {
                                    ui.label(self.i18n.t("recorder.batch_label"));
                                    let bs_resp = ui.add(egui::TextEdit::singleline(&mut self.recorder_batch_size_edit).desired_width(theme.input_w_small));
                                    ui.label(self.i18n.t("recorder.flush_label"));
                                    let fi_resp = ui.add(egui::TextEdit::singleline(&mut self.recorder_flush_interval_ms_edit).desired_width(theme.input_w_small));
